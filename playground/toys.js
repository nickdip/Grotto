    ...
    words.forEach((word) => {
      ...
      this.toyData.forEach((toy) => {
        if (word === toy.Category) {
          this.favCategories[toy.Category] =
            this.favCategories[toy.Category] + 1 || 1;
        }
        ...
        if (toy.words.includes(word)) {
          ...
          if (
            !(
              matches.includes(toy) ||
              this.possibleToys.Primary.includes(toy) ||
              this.possibleToys.Secondary.includes(toy)
            )
          )
            matches.push(toy);
        }
    ...
    this.words.forEach((wordInText) => {
      spellcheck.getCorrections(wordInText, 1).forEach((correction) => {
        if (correction !== wordInText && secondaryMatches.includes(correction))
          this.secondaryWords.push(correction);
      });
    });
